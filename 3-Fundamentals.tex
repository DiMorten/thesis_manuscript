...
TTA is different from the previous approaches because it estimates the so-called aleatoric (data) uncertainty caused by variations in the input caused by various noise sources. MCD and ensembles capture uncertainty linked to the models themselves, the so-called epistemic or model uncertainty. %TTA is different from the previous approaches because it estimates the so-called aleatoric (data) uncertainty, caused by variations/noise in the input. MCD and ensembles capture uncertainty linked to the models themselves, the so-called epistemic or model uncertainty. 
